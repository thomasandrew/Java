...
public class Pessoa {
	...
	public static void validarCpf(String cpf) throws Exception {
		...
		if(cpf == null || cpf.length() == 0)
			throw new Exception("O cpf de uma pessoa não pode ser nulo!");
		// Verificando se o tamanho (em caracteres) da String do cpf é igual a 14
		if(cpf.length() != 14)
			throw new Exception("O cpf deve 14 caracteres no formato 999.999.999-99!");
		...
		for(int i = 0; i < cpf.length(); i++) {
			...
			default:
				if(! Character.isDigit(c))
					throw new Exception("O caracter na posição " + i + " deve corresponder a um dígito: " + c);
				break;
			}
		}
	}
	...
	public static void validarNome(String nome) throws Exception {
		...
		if(nome == null || nome.length() == 0)
			throw new Exception("O nome de uma pessoa não pode ser nulo!");
		// Verificando o tamanho (em caracteres) da String
		if(nome.length() < 5 || nome.length() > 40)
			throw new Exception("O nome deve ter entre 5 e 40 caracteres!");
		// Verificando cada um dos caracteres presentes na String
		for(int i = 0; i < nome.length(); i++) {
			...
			if( ! Character.isAlphabetic(c) && ! Character.isSpaceChar(c) )
				throw new Exception("O caracter na posição " + i + " é inválido: " + c);
		}
	}
